...
//    public int[][] compartmentAvailableSeats;
	...
	private void combinationUtil(List<List<CompartmentSeat>> combarr, List<CompartmentSeat> arr,
			List<CompartmentSeat> data, int start, int end, int index, int r, int[] pref) {
		if (index == r) {
	...
	public List<CompartmentSeat> findSeatAvailability(List<CompartmentSeat> arr, int[] pref, String className,
			int noOfSeats) {
...
//         Check the all possible combination for available with preference array
		for (List<CompartmentSeat> a : combarr) {
...
//        System.out.println("Final pref "+finalprefarr.toString());
		if (minimumDistanceSeats(finalprefarr))
			return finalprefarr.get(0);

		// if prefered seats are not available send to retrieve the
		// After removing preference checks all possible combination and then add it to
		// the final array
		int[] newpref = { 0, 0, 0 };
		for (List<CompartmentSeat> a : combarr) {
...
//        System.out.println("Final Pref"+finalprefarr);
	...
	private boolean minimumDistanceSeats(List<List<CompartmentSeat>> finalprefarr) {
		...
		if (finalprefarr.size() != 0) {
			...
			for (int i = 0; i < finalprefarr.size(); i++) {
				int distance = finalprefarr.get(i).get(finalprefarr.get(i).size() - 1).getS()
						- finalprefarr.get(i).get(0).getS();
	...
	public List<CompartmentSeat> Book(List<List<CompartmentSeat>> availableSeats, int[] prefs, String className,
			int noOfSeats) {
		int i = 1;
		int minimumdist = Byte.MAX_VALUE;
		for (List<CompartmentSeat> compartment : availableSeats) {
		...
		if (seatAvailability.isEmpty()) {
			...
			for (int j = 0; ind < noOfSeats && availableSeats.size() > 0 && j < availableSeats.size(); j++) {
				for (int k = 0; ind < noOfSeats && availableSeats.get(j).size() > 0
						&& k < availableSeats.get(j).size(); k++) {
